  ...
  const DetailItem = ({ icon: Icon, label, value, color }: any) => (
    <div className="flex items-center gap-4 group">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[9px] font-black text-slate-400 leading-none mb-1">{label}</p>
        <p className="text-sm font-black text-slate-900 tracking-tight leading-tight truncate">{value}</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen pb-20 space-y-10 max-w-6xl mx-auto px-4 pt-6">
      ...
      <div className="flex flex-col gap-6">
        <Link href="/tournaments" className="inline-flex items-center gap-2 text-slate-400 hover:text-slate-900 font-black text-[10px] transition-colors w-fit">
          <ArrowLeft size={14} /> Back to Hub
        </Link>

        <div className="space-y-4">
          <div className={`inline-flex px-4 py-1.5 rounded-full border text-[10px] font-black ${
          ...
          <h1 className="text-4xl sm:text-7xl font-black text-slate-900 tracking-tighter leading-[0.9] max-w-4xl">
            {tournament.name}
          </h1>
        ...
        <div className="lg:col-span-7 space-y-12">
          ...
          <div className="space-y-6 px-2">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-black text-brand-orange">Engagement Protocol</span>
            </div>
            <div className="space-y-4">
          ...
          <div className="flex bg-white/70 backdrop-blur-md p-1.5 rounded-2xl border border-white shadow-xl">
            {[
              ...
              <button
                ...
                className={`flex-1 px-4 py-3 rounded-xl text-[10px] font-black transition-all flex items-center justify-center gap-2 ${
                  activeTab === tab.id 
            ...
            {activeTab === 'info' && (
              ...
              >
                ...
                <div className="bg-white/70 backdrop-blur-3xl border border-white p-8 sm:p-10 rounded-[2.5rem] sm:rounded-[3.5rem] shadow-2xl relative overflow-hidden group">
                  ...
                  <h3 className="text-[10px] font-black text-slate-400 mb-10 flex items-center gap-3">
                  ...
                  <div className="space-y-8 relative z-10">
                    ...
                    <div className="pt-8 border-t border-slate-100 space-y-6">
                      <h4 className="text-[9px] font-black text-slate-400">Organizer Identity</h4>
                      <div className="space-y-4">
                        <DetailItem 
                          icon={Phone} 
                          label="Phone Number" 
                ...
                <div className="space-y-4">
                  {tournament.status === 'UPCOMING' ? (
                    <Link href={session ? `/register?tournamentId=${tournament.id}` : "/login?mode=signup"}>
                      <Button className="w-full h-20 bg-brand-orange hover:bg-slate-900 text-white font-black text-sm shadow-[0_20px_40px_-10px_rgba(255,107,1,0.2)] transition-all rounded-[2rem] group relative overflow-hidden">
                    ...
                    <div className="w-full p-6 bg-white/70 backdrop-blur-xl rounded-[2rem] border border-white text-center shadow-xl">
                      <p className="text-xs font-black text-slate-400 italic opacity-60">Registration Protocol Offline</p>
                    </div>
                  )}

                  {isOrganizer && (
                    <Link href={`/admin?tournamentId=${tournament.id}`} className="block">
                      <Button variant="ghost" className="w-full h-16 bg-white hover:bg-slate-50 text-slate-900 font-black text-xs rounded-[1.5rem] border border-slate-200 shadow-sm transition-all">
              ...
              >
                {(!tournament.matches || tournament.matches.length === 0) ? (
                  <div className="py-20 text-center bg-white/70 backdrop-blur-xl rounded-[2.5rem] border-2 border-dashed border-slate-200 shadow-xl">
                    <p className="text-slate-400 font-black text-[10px] opacity-60">Pairings Not Generated</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {Object.entries(
                      ...
                      <div key={round} className="bg-white/70 backdrop-blur-xl border border-white rounded-[2rem] overflow-hidden shadow-xl">
                        ...
                        >
                          <span className="text-sm font-black text-slate-900">Round {round}</span>
                            ...
                            <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden px-4 pb-4">
                              <div className="space-y-2">
                                ...
                                {[...matches].sort((a: any, b: any) => {
                                  ...
                                  <div key={m.id} className="bg-slate-50 p-4 rounded-xl flex justify-between items-center text-[10px] font-black border border-slate-100">
                  ...
                  <table className="w-full text-left border-collapse min-w-full">
                    <thead className="bg-slate-50 border-b border-slate-100">
                      <tr className="text-[9px] font-black text-slate-400">
                        <th className="px-4 sm:px-6 py-4">Pos</th>
                        <th className="px-4 sm:px-6 py-4">Player</th>
                        <th className="px-4 sm:px-6 py-4 text-center hidden sm:table-cell">Online</th>
                        <th className="px-4 sm:px-6 py-4 text-right">Pts</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                    {tournament.players?.sort((a: any, b: any) => b.score - a.score).map((p: any, i: number) => {
                          ...
                          return (
                            <React.Fragment key={p.id}>
                              ...
                              >
                                <td className="px-4 sm:px-6 py-4 text-[10px] font-black text-slate-400">
                                  {tournament.status === 'FINISHED' && i < 3 ? (
                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs shadow-sm ${
                                      i === 0 ? 'bg-yellow-400 text-yellow-950 border border-yellow-500' :
                                      i === 1 ? 'bg-slate-300 text-slate-900 border border-slate-400' :
                                ...
                                <td className="px-4 sm:px-6 py-4">
                                  <div className="flex flex-col min-w-0">
                                    <span className="text-[10px] sm:text-xs font-black text-slate-900 tracking-tight leading-tight truncate max-w-[140px] sm:max-w-[200px]">
                                      {p.fullName}
                                    </span>
                                    <div className="flex items-center gap-2 mt-0.5">
                                      ...
                                      <span className="sm:hidden text-[8px] font-black text-blue-500">{p.externalRating || '—'}</span>
                                    </div>
                                  </div>
                                </td>
                                <td className="px-4 sm:px-6 py-4 text-[10px] font-black text-blue-600 text-center hidden sm:table-cell">{p.externalRating || '—'}</td>
                                <td className="px-4 sm:px-6 py-4 text-right text-brand-orange font-black text-sm sm:text-base">{p.score}</td>
                              </tr>
                              <AnimatePresence>
                                {isExpanded && (
                                  <tr>
                                    <td colSpan={4} className="px-4 sm:px-6 pb-4 pt-0">
                                      <motion.div
                                        initial={{ height: 0, opacity: 0 }}
                                        animate={{ height: 'auto', opacity: 1 }}
                                        exit={{ height: 0, opacity: 0 }}
                                        className="overflow-hidden bg-white/50 border border-slate-100 rounded-2xl p-4 space-y-4"
                                      >
                                        <div className="flex items-center justify-between pb-2 border-b border-slate-100/50">
                                          <div className="text-right ml-auto">
                                            <span className="text-[7px] font-black text-slate-400">Online Rating</span>
                                            <p className="text-xs font-black text-emerald-600">{p.externalRating || Math.round(p.rating)}</p>
                                          </div>
                                        </div>
                                      ...
                                      <div className="flex flex-wrap gap-2">
                                        {roundsString.map((res, idx) => (
                                          <div key={idx} className="flex flex-col items-center min-w-[32px]">
                                            <span className="text-[7px] font-black text-slate-400 mb-1">R{idx + 1}</span>
                                      ...
                                      <div className="pt-2 space-y-1.5 border-t border-slate-100/50">
                                        {playerMatches.map((m: any) => {
                                          ...
                                          return (
                                            <div key={m.id} className="flex justify-between items-center text-[8px] font-black text-slate-500 gap-4">
                                              <span className="truncate flex-1 text-left">RD {m.round} vs {opponent || 'BYE'} ({isWhite ? 'W' : 'B'})</span>
                                              <span className={m.result ? 'text-slate-900 shrink-0' : 'text-slate-300 shrink-0'}>{m.result || 'PENDING'}</span>
                                            </div>
                                          );
                                        })}
                                      </div>
